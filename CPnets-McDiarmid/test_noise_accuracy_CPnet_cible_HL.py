...
for i in percentageOfNoise:
	fileTestNoiseHL[i] = open("test-results/test_acc_CPnet_cible_noise" + str(i) + "_Offline.dat","w")
for numberOfParentsForTargetCPNet in [0,1,2,4,6,8,11]:
	...
	for i in percentageOfNoise:
		fileTestNoiseHL[i].write(str(numberOfParentsForTargetCPNet) + " " + str(aOffline[i]) + " " + str(sdAOffline[i]) + " " + str(aOfflineLog[i]) + " " + str(sdAOfflineLog[i]) + " " + str(meanAccNoiseOffline[i]) + " " + str(sdANoiseOffline[i]) + " " + str(meanAccNoiseOfflineLog[i]) + " " + str(sdANoiseOfflineLog[i]) + "\n")
for i in percentageOfNoise:
