...
for numberOfParentsForLearnedCPNet in [0,1,2,4,6,8,11]:
	...
	for i in percentageOfNoise:
		fileTestNoiseOL[i].write(str(numberOfParentsForLearnedCPNet) + " " + str(aOnline[i]) + " " + str(sdAOnline[i]) + " " + str(aOnlineLog[i]) + " " + str(sdAOnlineLog[i]) + " " + str(meanAccNoiseOnline[i]) + " " + str(sdANoiseOnline[i]) + " " + str(meanAccNoiseOnlineLog[i]) + " " + str(sdANoiseOnlineLog[i]) + "\n")
for i in percentageOfNoise:
